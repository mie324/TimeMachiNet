...
def train(args):
    ...
    # create csv file to keep track of losses of networks
    ...
    # use CUDA for GPU usage
    cuda_available = torch.cuda.is_available()

    # create labelled folder for dataset
    create_destination()

    # move dataset into organized folders
    move_dataset()

    # load images
    image_loader = get_data_loader()

    # load models/networks
    if cuda_available:
        netD_z = Dzs().cuda()
        netE = Encoder().cuda()
        netG = Generator().cuda()
    ...
    # for validation (the age vectors)
    ...
    for epoch in range(args.epochs):
        for i, (img, label) in enumerate(image_loader):
            for j in range(len(label)):
                # 2*(age_bucket) + (0 or 1) (0: male, 1: female)
            ...
            # set up validation image for first epoch and step
            if epoch == 0 and i == 0:
            ...
            z = netE(img)
            reconst = netG(z, age_one_hot, gender_var)  # generated image
            EG_L1_loss = L1(reconst, img)  # mean abs error (makes more realistic)

            # EG_loss 2. GAN loss - image
            z = netE(img)
            reconstruction = netG(z, age_one_hot, gender_var)  # generated image
            D_reconstruction, _ = netD_img(reconstruction, age_one_hot.view(batch_size, n_l, 1, 1),
                                           gender_var.view(batch_size, 1, 1, 1))  # D(G(z))
            G_img_loss = BCE(D_reconstruction, real_label)  # log(D(G(z))

            # EG_loss 3. GAN loss - z
            Dz = netD_z(z)
            Ez_loss = BCE(Dz, real_label)  # log(D(z))

            # EG_loss 4. loss - G
            reconstruction = netG(z.detach(), age_one_hot, gender_var)
            G_tv_loss = compute_loss(reconstruction)  # total variance loss (de-noise)

            EG_loss = EG_L1_loss + 0.0001 * G_img_loss + 0.01 * Ez_loss + G_tv_loss  # weighed loss
            EG_loss.backward()
            ...
            Dz = netD_z(z.detach())  # detach() avoid training D on these labels
